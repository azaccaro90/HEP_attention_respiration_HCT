...
% cardiac interoception circular statistics
...
median_HEP = quantile(event_ecg_frequencies(:,3), [0.50]);

low_HEP = find(event_ecg_frequencies(:,3) <= median_HEP);
high_HEP = find(event_ecg_frequencies(:,3) >= median_HEP);
...
[p_low_HEP_otest, M_low_HEP_otest] = circ_otest(table_low_HEP_mean_direction(:,1));
[p_high_HEP_otest, M_high_HEP_otest] = circ_otest(table_high_HEP_mean_direction(:,1));
