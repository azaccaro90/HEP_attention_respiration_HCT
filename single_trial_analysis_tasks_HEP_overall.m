...
sig_chan = [1, 3, 17, 20, 21, 33, 34, 39, 40, 41, 71]; % significant channels interaction task * condition
...
%         event_ecg_frequencies_tab(j,6) = {['exteroception']};
    ...
    if  respiratory_intervals(k,1) < event_ecg_frequencies(j,1) && event_ecg_frequencies(j,1) < respiratory_intervals(k,2)
        event_ecg_frequencies_tab(j,5) = {['respiratory']};
        event_ecg_frequencies_tab(j,6) = {['interoception']};
%         event_ecg_frequencies_tab(j,6) = {['exteroception']};
    end
...
xlswrite('table_single_trials_cardiac_vs_respiratory_interoception.xlsx', table_single_trials);
% xlswrite('table_single_trials_cardiac_vs_respiratory_exteroception.xlsx', table_single_trials);
