...
% OPEN EEGLAB, import EEG data interoception_prunedICA.set

% single trial analysis among phases during the HCT
...
sig_chan = [2, 20, 22, 34, 61]; % significant channels inhale vs. exhale
...
for i = 1:length(event_ecg_frequencies_tab)
    ...
    event_ecg_frequencies_tab {i,8} = z;
...
table_single_trials(:,1) = table_single_trials(:,8);
table_single_trials(:,8) = [];

% export table for statistics
